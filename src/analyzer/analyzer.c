      ...
      if (strcmp(newPath, "///") != 0) {
        ...
        if (rc_an != SUCCESS) {
          ...
          if (type == DIRECTORY || type == IS_FILE) {
            ...
            if (rc_ntnc != SUCCESS) {
              fprintf(stderr,
                      "entro nell'errorHandler in RDL pt 6 con codice %d\n",
                      rc_ntnc);
            ...
            if (rc_t == SUCCESS && toAnalyze != NULL) {
              ...
              if (rc_pu != SUCCESS) {
                fprintf(stderr,
                        "entro nell'errorHandler in RDL pt 7 con codice %d\n",
                        rc_pu);
    ...
    if (m != NULL) {
      if (amount > 0) {
        if (m != NULL) {
          ...
          if (rc_em != SUCCESS) {
            fprintf(stderr,
                    "entro nell'errorHandler in RM pt 1 con codice %d\n",
                    rc_em);
          ...
          if (skipped >= candidate->toSkip) {
            if (charRead == '\n' || charRead == '\0') {
              ...
              if (counter > 0) {
                ...
                if (rc_ia != SUCCESS) {
                  fprintf(stderr,
                          "entro nell'errorHandler in FML pt 4 con codice %d\n",
                          rc_ia);
    ...
    while (nManager > 0 && rc_t == SUCCESS) {
      ...
      if (manager != NULL) {
        ...
        if (isAliveM == SUCCESS) {
          ...
          if (bytesRead > 0) {
            ...
            if (rc_t != SUCCESS) {
              ...
              if (isManagerAlive(manager) == SUCCESS) {
                  ...
                  if (bytesRead > 0) {
                    if (strcmp(controlWord, CONTROL_DONE) == 0) {
                      if (found == 1) {
                        ...
                        if (rc_dn != SUCCESS) {
                          fprintf(stderr,
                                  "entro nell'errorHandler in SFL pt 6 con "
                                  "codice %d\n",
                                  rc_dn);
                ...
                if (rc_rm != SUCCESS) {
                  fprintf(stderr,
                          "entro nell'errorHandler in SFL pt 8 con codice %d\n",
                          rc_rm);
                  rc_t = errorHandler(rc_rm);
                } else {
          ...
          if (rc_rm != SUCCESS) {
            fprintf(stderr,
                    "entro nell'errorHandler in SFL pt 10 con codice %d\n",
                    rc_rm);
            rc_t = errorHandler(rc_rm);
          } else {
    ...
    if (isEmptyList(sharedResources->fileToAssign) == NOT_EMPTY) {
      ...
      rc_mfs = manageFileToSend(sharedResources->managers,
                                *(sharedResources->nWorker),
                                sharedResources->fileToAssign);
      if (rc_mfs != SUCCESS) {
        // fprintf(stderr, "entro nell'errorHandler in SFL pt 16 con codice
        // %d\n", rc_mfs);
        errorHandler(rc_mfs);
      }
    }
          ...
          if (strcmp(dst, "dire") == 0) {
            ...
            if (rc_t == SUCCESS) {
              if (strcmp(newPath, "///") != 0) {
                ...
                if (rc_an != SUCCESS) {
                  fprintf(stderr,
                          "entro nell'errorHandler in RFF pt 9 con codice %d\n",
                          rc_an);
                  rc_t = errorHandler(rc_an);
                } else {
                  if (type == DIRECTORY) {
                  ...
                  if (type == DIRECTORY || type == IS_FILE) {
                    ...
                    if (rc_ntnc != SUCCESS) {
                      fprintf(stderr,
                              "entro nell'errorHandler in RFF pt 10 con codice "
                              "%d\n",
                              rc_ntnc);
                      rc_t = errorHandler(rc_ntnc);
                    }
                    if (toAnalyze != NULL) {
                      ...
                      if (rc_pu != SUCCESS) {
                        fprintf(stderr,
                                "entro nell'errorHandler in RFF pt 11\n");
                        rc_t = errorHandler(MALLOC_FAILURE);
                      }
                    } else {
                    ...
                    if (type == NOT_EXISTING) {
                      ...
                      if (rc_al2 < SUCCESS) {
                        fprintf(stderr,
                                "entro nell'errorHandler in RFF pt 13\n");
                        rc_t = errorHandler(MALLOC_FAILURE);
                      } else {
                        sprintf(msgInfo, "The File %s doesn't exist", newPath);
                        printInfo(msgInfo);
            ...
            if (rc_rqf != SUCCESS) {
              fprintf(stderr,
                      "entro nell'errorHandler in RFF pt 15 con codice %d\n",
                      rc_rqf);
              ...
              if (requestedPath != NULL) {
                ...
                if (rc_pi == SUCCESS ||
                  ...
                  if (requestedFile != NULL) {
                    ...
                    if (rc_st != SUCCESS) {
                      fprintf(stderr,
                              "entro nell'errorHandler in RFF pt 16 con codice "
                              "%d\n",
                              rc_st);
    ...
    if (sharedResources->toRetrieve != NULL) {
      if (fd > 0) {
        ...
        if (msg == SUCCESS || (msg == ALREADY_INSERTED && requested != NULL)) {
          ...
          if (rc_ctr != SUCCESS) {
            fprintf(stderr,
                    "entro nell'errorHandler in WFL pt 1 con codice %d\n",
                    rc_ctr);
    ...
    if (rc_ir == SUCCESS) {
      ...
      if (rc_it == SUCCESS) {
        ...
        if (rc_en == SUCCESS) {
          ...
          if (rc_pi != SUCCESS) {
            fprintf(stderr,
                    "entro nell'errorHandler in Main pt 7 con codice %d\n",
                    rc_pi);
            rc_t = errorHandler(rc_pi);
          }
        } else {
          fprintf(stderr,
                  "entro nell'errorHandler in Main pt 8 con codice %d\n",
                  rc_en);
